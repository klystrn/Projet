...
(function () {
  ...
  /* ---------------- hero dashboard summary ----------------
     BACKEND HOOK (Andrei). #heroDash carries data-endpoint. While it is
     empty the card shows sample values behind a signed-out gate, so a
     logged-out visitor never reads the numbers as their own. Point
     data-endpoint at a route returning the signed-in user's summary and
     this fills the same slots and drops the gate.

     Expected response shape (suggested, adjust freely — the front end reads
     these keys and nothing else):
       {
         ...
         "sub": "SUTD · Year 3",
         "chip": "3 entries",
         "metrics": [{ "value": "83", "label": "Avg. score" }, ...],
         "rows": [{ "rank": "04", "title": "...", "meta": "...", "value": "85" }, ...],
         "footNote": "Your last 3 challenges"
       } */
  (function () {
    var card = document.getElementById("heroDash");
    var gate = document.getElementById("dashGate");
    if (!card) return;
    var endpoint = card.getAttribute("data-endpoint");
    if (!endpoint) return; // sample data + gate stay as authored

    fetch(endpoint, { credentials: "same-origin" })
      .then(function (r) { return r.ok ? r.json() : null; })
      .then(function (data) {
        if (!data) return;
        var inner = card.querySelector(".dash-inner");
        if (!inner) return;
        var set = function (sel, text) {
          var el = inner.querySelector(sel);
          if (el && text != null) el.textContent = text;
        };
        set(".dash-who", data.name);
        set(".dash-role", data.sub);
        set(".dash-chip", data.chip);
        if (Array.isArray(data.metrics)) {
          inner.querySelectorAll(".dash-metric").forEach(function (m, i) {
            var d = data.metrics[i];
            if (!d) return;
            var b = m.querySelector("b"), s = m.querySelector("span");
            if (b && d.value != null) b.textContent = d.value;
            if (s && d.label != null) s.textContent = d.label;
          });
        }
        if (Array.isArray(data.rows)) {
          inner.querySelectorAll(".dash-row").forEach(function (row, i) {
            var d = data.rows[i];
            if (!d) return;
            set0(row, ".dash-rank", d.rank);
            set0(row, ".dash-row-title", d.title);
            set0(row, ".dash-row-meta", d.meta);
            set0(row, ".dash-row-val", d.value);
          });
        }
        set(".dash-foot-note", data.footNote);
        if (gate) gate.remove(); // a real session is loaded; stop gating it
      })
      .catch(function () { /* leave the sample card and its gate in place */ });

    function set0(scope, sel, text) {
